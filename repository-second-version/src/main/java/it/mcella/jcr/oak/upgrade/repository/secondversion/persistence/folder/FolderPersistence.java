...
public class FolderPersistence {
    ...
    public FolderNode createIntoRoot(String folderName, String description) throws FolderPersistenceException {
        try (JcrSession jcrSession = jcrRepository.login(AdministratorCredentials.create())) {
            JcrNode jcrRootNode = jcrSession.getRootNode();
            JcrFolderNode jcrFolderNode = createFolderNode(jcrRootNode, folderName, description);
            jcrSession.save();
    ...
    public FolderNode createInto(FolderNode parentFolderNode, String folderName, String description) throws FolderPersistenceException {
        return createInto(parentFolderNode.getJcrNodeId(), folderName, description);
    }

    public FolderNode createInto(JcrNodeId jcrParentNodeId, String folderName, String description) throws FolderPersistenceException {
        try (JcrSession jcrSession = jcrRepository.login(AdministratorCredentials.create())) {
            JcrNode jcrParentFolderNode = jcrSession.getNodeById(jcrParentNodeId);
            JcrFolderNode jcrFolderNode = createFolderNode(jcrParentFolderNode, folderName, description);
            jcrSession.save();
            FolderNode folderNode = jcrFolderNode.toNode();
            LOGGER.info("Created folder:" + folderNode);
            return folderNode;
    ...
    private JcrFolderNode createFolderNode(JcrNode jcrParentFolderNode, String folderName, String description) throws RepositoryException {
        ...
        jcrFolderNode.setDescription(description);
